...
const ProjectModal = ({ project, onClose, language }: ProjectModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fade-in">
      ...
      <div className="relative w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-3xl bg-white dark:bg-gradient-to-br dark:from-[#0a0a1a] dark:to-[#1a0a2e] border border-gray-300 dark:border-purple-500/30 shadow-2xl shadow-purple-500/20 animate-scale-in">
        <button
          onClick={onClose}
          className="absolute top-6 right-6 z-10 p-3 rounded-full bg-gray-200 dark:bg-white/10 hover:bg-gray-300 dark:hover:bg-white/20 transition-colors"
        >
        ...
        <div className={`h-64 bg-gradient-to-br ${project.color} relative overflow-hidden`}>
          ...
          <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-white to-transparent dark:from-[#0a0a1a] dark:to-transparent"></div>
        </div>

        <div className="p-8 space-y-8">
          <div>
            <p className="text-xl text-gray-900 dark:text-white/90 leading-relaxed mb-4">
          ...
          {hasGallery && (
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <Image className="text-cyan-600 dark:text-cyan-400" size={24} />
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{t.gallery}</h3>
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="p-6 rounded-2xl bg-gradient-to-br from-purple-100/50 to-pink-100/50 dark:from-purple-500/10 dark:to-pink-500/10 border border-gray-300 dark:border-white/10">
              <h3 className="text-lg font-semibold text-cyan-600 dark:text-cyan-400 mb-3">{t.challenge}</h3>
              <p className="text-gray-700 dark:text-white/70">{project.challenge}</p>
            </div>

            <div className="p-6 rounded-2xl bg-gradient-to-br from-purple-100/50 to-pink-100/50 dark:from-purple-500/10 dark:to-pink-500/10 border border-gray-300 dark:border-white/10">
              <h3 className="text-lg font-semibold text-cyan-600 dark:text-cyan-400 mb-3">{t.solution}</h3>
              <p className="text-gray-700 dark:text-white/70">{project.solution}</p>
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-br from-purple-100/50 to-pink-100/50 dark:from-purple-500/10 dark:to-pink-500/10 border border-gray-300 dark:border-white/10">
            ...
            <p className="text-gray-700 dark:text-white/70">{project.role}</p>
          </div>

          <div>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">{t.features}</h3>
            <ul className="space-y-3">
              {project.features.map((feature, index) => (
                <li key={index} className="flex items-start gap-3 text-gray-700 dark:text-white/80">
          ...
          <div>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">{t.technologies}</h3>
            <div className="flex flex-wrap gap-3">
              {project.technologies.map((tech, index) => (
                <span
                  key={index}
                  className="px-4 py-2 rounded-xl bg-gradient-to-r from-purple-100 to-pink-100 dark:from-purple-500/20 dark:to-pink-500/20 border border-purple-400/40 dark:border-purple-500/30 text-gray-900 dark:text-white/90 font-medium"
                >
                  {tech}
                </span>
              ))}
          ...
          <div className="flex flex-wrap gap-4 pt-4">
            ...
            {project.github && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="group px-8 py-4 bg-gray-200 dark:bg-white/10 backdrop-blur-sm text-gray-900 dark:text-white rounded-full font-semibold border border-gray-300 dark:border-white/20 hover:bg-gray-300 dark:hover:bg-white/20 hover:shadow-xl transition-all duration-300 hover:scale-105 flex items-center gap-2"
              >
                {t.viewCode}
